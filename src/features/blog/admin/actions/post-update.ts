/**
 * Post Update Action
 *
 * Handles updating an existing post from form submission
 */

import type { Context } from "hono";
import { getPostById, updatePost } from "@/db/queries/posts.js";
import { getAllCategories } from "@/db/queries/categories.js";
import { isValidSlug } from "../../shared/utils.js";
import { PostForm } from "../views/post-form.js";

export async function postUpdateAction(c: Context) {
	const id = Number.parseInt(c.req.param("id"));
	const formData = await c.req.formData();

	// Extract form fields
	const title = formData.get("title") as string;
	const slug = formData.get("slug") as string;
	const content = formData.get("content") as string;
	const excerpt = formData.get("excerpt") as string | null;
	const categoryIdParam = formData.get("categoryId") as string;
	const status = formData.get("status") as "draft" | "published";

	// Validate input
	const errors: Record<string, string> = {};

	if (!title || title.trim().length === 0) {
		errors.title = "Title is required";
	}

	if (!slug || slug.trim().length === 0) {
		errors.slug = "Slug is required";
	} else if (!isValidSlug(slug)) {
		errors.slug = "Slug must contain only lowercase letters, numbers, and hyphens";
	}

	if (!content || content.trim().length === 0) {
		errors.content = "Content is required";
	}

	// If there are validation errors, redisplay the form
	if (Object.keys(errors).length > 0) {
		const postWithRelations = await getPostById(id);
		const categories = await getAllCategories();
		return c.html(
			<PostForm
				post={postWithRelations}
				categories={categories}
				attachments={postWithRelations?.attachments}
				errors={errors}
			/>,
			422,
		);
	}

	const categoryId = categoryIdParam && categoryIdParam !== ""
		? Number.parseInt(categoryIdParam)
		: null;

	try {
		// Update the post
		const updatedPost = await updatePost(id, {
			title: title.trim(),
			slug: slug.trim(),
			content: content.trim(),
			excerpt: excerpt?.trim() || null,
			categoryId,
			status,
			publishedAt: status === "published" ? new Date() : null,
		});

		if (!updatedPost) {
			return c.notFound();
		}

		// TODO: Handle file uploads (images and downloads)

		// Redirect back to the post list
		return c.redirect("/admin/blog");
	} catch (error) {
		console.error("Error updating post:", error);
		const postWithRelations = await getPostById(id);
		const categories = await getAllCategories();
		return c.html(
			<PostForm
				post={postWithRelations}
				categories={categories}
				attachments={postWithRelations?.attachments}
				errors={{ _general: "An error occurred while updating the post" }}
			/>,
			500,
		);
	}
}
